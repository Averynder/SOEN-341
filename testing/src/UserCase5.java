...
//		System.out.println("Clicked on generate sequence");
		...
		for (int i = 0; i< courses.length; i++) {
			for (int j = 0; j < courses[i].length; j++) {
				...
				WebElement fallTable = driver.findElement(By.id("pdfTable fallTable"));
				WebElement winterTable = driver.findElement(By.id("pdfTable winterTable"));
				WebElement[] tableArray = {fallTable, winterTable};
				if (tableArray[i].getText().contains(courses[i][j]))
