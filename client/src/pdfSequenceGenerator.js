...
class pdfSequenceGenerator extends React.Component{
    ...
    addClass = () => {
        ...
        let classExists = false;
            ...
            for(let i=0; i<fall.length; i++){ //This loop prevents duplicates for fall table
                if(fall[i].course === input && fall[i].semester === semester){
                    errorMessage.innerHTML = "You have already added this class";
                    return;
                }
            }
            for(let i=0; i<winter.length; i++){ //This loop prevents duplicates for winter table
                if(winter[i].course === input && winter[i].semester === semester){
                    errorMessage.innerHTML = "You have already added this class";
                    return;
                }
            }
            for(let i=0; i<summer.length; i++){ //This loop prevents duplicates for summer table
            ...
            for(let i=0; i<classList.length; i++){ // Finds if input class exists and stores it in addedClass
        ...
        switch(addedClass.semester){
            case "Fall": fall.push(addedClass);
                         this.setState({
                            selectedCoursesFall: fall,
                            showAdd: !this.state.showAdd,
                         });
                         break;
            case "Summer": summer.push(addedClass);
                           this.setState({
                               selectedCoursesSummer: summer,
                               showAdd: !this.state.showAdd,
                           });
                         break;
            case "Winter": winter.push(addedClass);
                           this.setState({
    ...
    removeClass = () => {
        ...
        winter = winter.filter(element => element.course !== removedWinterClass.value);   //Remove json course object from winter array
        summer = summer.filter(element => element.course !== removedSummerClass.value);   //Remove json course object from summer array
        this.setState({
            selectedCoursesFall: fall,
            selectedCoursesWinter: winter,
            selectedCoursesSummer: summer,
    ...
    render(){
        let falltable = <Table id="pdfTable" striped bordered hover variant="dark">
            ...
            let summertable = <Table id="pdfTable" striped bordered hover variant="dark">
                                <thead>
                                    <tr>
                                    <th>Semester</th>
                                    <th>Course</th>
            ...
            let removeOptions = <Table striped border hover>
                                    <tr>
                                        <td>Fall</td>
                                        <td>Winter</td>
                                        <td>Summer</td>
            ...
            <div>
                ...
                <div className="container">
                    <div className="jumbotron j-greetings">
                        <h2 className="display-4">Sequence To PDF <br/> Year {this.state.selectYear ? "" : this.state.sequenceYear}</h2>
                        <hr color="#7e1530"/>
                        <p className="lead">
                            Click Add Course and try out COMP248, COMP232, SOEN228 or ENGR213 to test it out.<br/><br/>
